package com.example.gen.generator.utils;
...
import static com.example.gen.generator.utils.StringUtil.lineToHump;

/**
 * @ Author     ：XJH.
 * @ Date       ：Created in 9:09 2018/1/24/024.
 * @ Description：MybatisGeneratorUtil
 * @ Modified By：
 */
public class MybatisGeneratorUtil {
    ...
    public static void generator(
        ...
        //对mapper接口进行调整start
        File file = new File(targetProject + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/mapper");
        if (!file.exists()) {
            return;
        }

        StringBuilder itemContent;
        ...
        if (files != null && file.length() > 0) {
            for (File itemFile : files) {
                if (itemFile.isFile()) {
                    br = new BufferedReader(new FileReader(itemFile));//构造一个BufferedReader类来读取文件
                    itemContent = new StringBuilder();
                    while ((s = br.readLine()) != null) {//使用readLine方法，一次读一行
                        itemContent.append(System.lineSeparator() + s);
                    }
                    if (itemContent.length() > 0) {
                        String itemContentString = itemContent.substring(0, itemContent.indexOf("{") + 1);
                        itemContentString += ("\n\n}");
                        w = new FileWriter(itemFile, false);
                        w.write(itemContentString);
                        w.close();
                    }
                    br.close();
